...
import s from './ListaGarantia.module.css';

export const ListaGarantia = ({ totalFinal, setLoading, loading, selecciones, setSelecciones, desvanecer, setDesvanecer, irDireccion }) => {
    ...
    return (
    <div className={s.container__garantia}>
        <div className={`garantia  ${desvanecer ? 'fade-out' : 'fade-in'}`}>
            <h2 className={s.title__garantia}>Agrega una proteccion para este producto</h2>
    ...
    return (
        <div key={product.id} className={s.garantia__tipo}>
            <div className={s.garantia__product}>
                <div className={s.container__image}>
                    <img className={s.image__garantia} src={product.image} alt={product.title} />
                </div>
                <div className={s.proteccion__product}>
                    <p className={s.proteccion__para}>proteccion para:</p>
                    <p className={s.proteccion__title}>{product.title}</p>
                </div>
            </div>
            ...
            <div className={`opcion ${selecciones[product.id]?.tipo === '12' ? 'active' : ''}`} onClick={() => seleccionarGarantia(product.id, 50, '12')}>
                ...
                <span className={s.garantia__precio}>$ 50</span>
            </div>
            ...
            <div className={`opcion ${selecciones[product.id]?.tipo === '18' ? 'active' : ''}`} onClick={()=> seleccionarGarantia(product.id, precio18, '18')}>
                ...
                <div className={s.garantia__meses}>
                    <span className={s.garantia__precio}>$ {precio18.toFixed(2)}</span>
                    <span className={s.garantia__descuento}>20% OFF</span>
                </div>
            </div>
            ...
            <div className={`opcion ${selecciones[product.id]?.tipo === '24' ? 'active' : ''}`} onClick={()=> seleccionarGarantia(product.id, precio24, '24')}>
                ...
                <div className={s.garantia__meses}>
                    <span className={s.garantia__precio}>$ {precio24.toFixed(2)}</span>
                    <span className={s.garantia__descuento}>10% OFF</span>
                </div>
            </div>
        </div>
            )
        ...
        <div className={s.btn__garantia}>
            ...
            <div className={s.condiciones__garantia}>
